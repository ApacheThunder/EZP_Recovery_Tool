...
u16 switchToTwlBlowfish(sNDSHeaderExt* ndsHeader) {
	if (twlBlowfish || ndsHeader->unitCode == 0) return ndsHeader->headerCRC16;
	...
	u16 twlHeaderCRC = 0xFFFF;
	
	// Read the header again and store header CRC so it can be checked later if needed.
	cardParamCommand (CARD_CMD_HEADER_READ, 0,
		CARD_ACTIVATE | CARD_nRESET | CARD_CLK_SLOW | CARD_BLK_SIZE(1) | CARD_DELAY1(0x1FFF) | CARD_DELAY2(0x3F),
		(void*)twlHeaderData, 0x200/sizeof(u32));
	
	twlHeaderCRC = ((sNDSHeaderExt*)twlHeaderData)->headerCRC16;
...
void cardRead (u32 src, void* dest, bool nandSave) {
	...
	if (src > ndsHeader->romSize && !(nandSave && src >= cardNandRwStart))switchToTwlBlowfish(ndsHeader);
}
...
void cardWriteNand (void* src, u32 dest) {
	...
	for (int i = 0; i < 4; i++)cardPolledTransferWrite(portFlags | CARD_ACTIVATE | CARD_WR | CARD_nRESET | CARD_BLK_SIZE(1), src + (i * 0x200), 0x200 / sizeof(u32), cmdData);
	
	cardParamCommand(CARD_CMD_NAND_COMMIT_BUFFER, 0, portFlags | CARD_ACTIVATE | CARD_nRESET, NULL, 0);

	u32 status;
	while((status & BIT(5)) == 0)cardParamCommand(CARD_CMD_NAND_READ_STATUS, 0, portFlags | CARD_ACTIVATE | CARD_nRESET | CARD_BLK_SIZE(7), &status, 1);
